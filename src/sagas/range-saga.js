...
function* calculateRedundantSizes(toRemoveIds) {
  ...
  return allRanges
    ...
    .reduce((acc, next) => {
      const filteredChildren = allRanges
        .filter(
          range =>
            // Not already removed
      ...
      const reducedChildren = sortedChildren.reduce(
        (ac, n) => {
          let push = false;
          if (n.startTime < ac.min) {
            ac.min = n.startTime;
            push = true;
          }
          if (n.endTime > ac.max) {
